<div align="center">

<img src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExcE53MDlhajVlCJ3O/giphy.gif" width="200"/>

# Hi 👋, I'm Anas
### Computer Science Student | Dev · Data · AI

[![LinkedIn](https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&label=&color=0077B5&logoColor=white&labelColor=&style=for-the-badge)](https://www.linkedin.com/in/anas-atertor-27838723b/)
[![Gmail](https://img.shields.io/static/v1?message=Gmail&logo=gmail&label=&color=D14836&logoColor=white&labelColor=&style=for-the-badge)](mailto:anas.ate10@gmail.com)


</div>

---

### 🌟 What I'm Passionate About

🤖 **AI & Automation** — Building intelligent multi-agent systems that turn complex problems into autonomous pipelines.

🌍 **Real-World Impact** — Engineering end-to-end solutions, from data ingestion to interactive dashboards deployed in production.

🔐 **Security-Aware Engineering** — Developing with a security-first mindset: authentication, access control, data governance.

🚀 **Entrepreneurial Spirit** — Excited by the intersection of tech and business — side projects, startups, and building things that matter.

💡 **Continuous Growth** — Always exploring new frameworks, tools, and paradigms to stay sharp.

---

### 🔥 What I'm Working On

- 🔭 **[Autoreach](https://github.com/at-ana)** — An intelligent multi-agent platform automating targeted lead generation: audience definition, web scraping, enrichment, and hyper-personalized outreach campaigns.
- 🎓 Preparing for **Master 1** (Sept 2026)
- 💼 Seeking a **24-month alternance** in Dev / Data / AI 

---

### 🛠 Languages & Tools

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="40" height="40" alt="Python"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40" height="40" alt="JavaScript"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="40" height="40" alt="Java"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="40" height="40" alt="PHP"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" width="40" height="40" alt="C"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="40" height="40" alt="PostgreSQL"/>
  &nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="40" height="40" alt="Linux"/>
  &nbsp;
  <img src="https://raw.githubusercontent.com/devicons/devicon/2ae2a900d2f041da66e950e4d48052658d850630/icons/pandas/pandas-original.svg" width="40" height="40" alt="Pandas"/>
  &nbsp;

---

### 📊 My Stats

<p align="center">
<img src="https://raw.githubusercontent.com/at-ana/at-ana/output/github-snake.svg" alt="snake animation" /></p>

---



<div align="center">
  <img src="https://raw.githubusercontent.com/at-ana/at-ana/output/github-contribution-grid-snake-dark.svg" alt="snake animation" />
</div>
 
